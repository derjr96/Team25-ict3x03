...
class SRPContext(object):
    ...
    * Using the Secure Remote Password (SRP) Protocol for TLS Authentication
    ...
    def get_common_secret(self, server_public, client_public):
        ...
        """
        #! CHANGE
        return self.hash(hex_from(self.pad(client_public)+self.pad(server_public)))
    ...
    def get_common_session_key(self, premaster_secret):
        ...
        """
        #! CHANGE
        return self.hash(hex_from(premaster_secret), as_bytes=True)
    ...
    def get_common_password_hash(self, salt):
        ...
        #! CHANGE
        return self.hash(salt, hex_from(self.hash(self._user, password, joiner=':', as_bytes=True)))
    ...
    def get_common_session_key_proof(self, session_key, salt, server_public, client_public):
        ...
        #! CHANGE
        h = self.hash
        x = h(self._prime) ^ h(self._gen)
        compute_first = hex_from(int_from_hex(hex_from(h(hex_from(self._prime))))^int_from_hex(hex_from(h(str(self._gen)))))
        prove = h(compute_first, hex_from(h(self._user)), hex_from(salt), hex_from(client_public), hex_from(server_public), hex_from(session_key), as_bytes=True)
        return prove
    ...
    def get_common_session_key_proof_hash(self, session_key, session_key_proof, client_public):
        ...
        """
        #! CHANGE
        return self.hash(hex_from(client_public), hex_from(session_key_proof), hex_from(session_key), as_bytes=True)
