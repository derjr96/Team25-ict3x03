$(document).ready(function () {
	$("#oldPwEye").on("touchstart mousedown", function (event) {
		event.stopImmediatePropagation();
		opMouseDown = true;
		setTimeout(resetEye, 1000);
		toggleEye(document.getElementById("oldPwEye"));
	});
	$("#oldPwEye").on("touchend mouseup", function (event) {
		event.stopImmediatePropagation();
		if (opMouseDown) {
			toggleEye(document.getElementById("oldPwEye"));
		} else {
    ...
    $("#btnConfirm").click(function(){
		if(formValid.every((e) => e === true)){
            ...
            document.getElementById("changePw-form").reportValidity();
        }
    });
});
let pMouseDown = false;
let cpMouseDown = false;
let opMouseDown = false;
let formValid = [false, false, false,false];
let doValidate = (el) => {
	const $this = $(el);
		...
		case "text":
			switch ($this.attr("id")) {
				...
				case "pw":
					...
					break;
				case "cpw":
					formValid[2] = validatePw($this.val());
					formValid[2] = setInputBehaviour($this, formValid[2]);
					break;
			}
			break;
		case "password":
...
let resetEye=()=>{
	if (pMouseDown){
		...
		$("#oldPwEye > svg").addClass("fa-eye");
		$("#oldPwEye > svg").removeClass("fa-eye-slash");
		pMouseDown = false;
	}
}
